...
import org.gearvrf.sutdvr.focus.ControlSceneObject;
import org.gearvrf.sutdvr.shaders.ButtonShader;

public class NavigationButton extends ControlSceneObject {

    private final int IDLE_STATE = 0;
    private final int HOVER_STATE = 1;
    private final int SELECTED_STATE = 2;
    private final int REMOVED_STATE = 3;

    private boolean removed;

    public NavigationButton(GVRContext gvrContext) {
        super(gvrContext);

        GVRMesh sMesh = getGVRContext().createQuad(0.05f, 0.05f);
    ...
    private void createTextures(GVRContext gvrContext) {
        ...
        getRenderData().getMaterial().setTexture(ButtonShader.STATE4_BACKGROUND_TEXTURE, empty);
        getRenderData().getMaterial().setTexture(ButtonShader.STATE4_TEXT_TEXTURE, empty);
    ...
    public void removeButton(){
        getRenderData().getMaterial().setFloat(ButtonShader.TEXTURE_SWITCH, REMOVED_STATE);
    ...
    public void rotateButton(float angle, float x, float y, float z){
